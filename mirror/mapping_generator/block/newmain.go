...
func main() {
	...
	nemcAirRID := 0
	skullRID := 0
	globalRuntimeIDData:=make([][3]interface{}, 0)
	err := json.Unmarshal(blockStateData, &globalRuntimeIDData)
	if err != nil {
		panic(err)
	}
	...
	blocks := []*RichBlock{}
	ridToMCBlock := []*GeneralBlock{}
	for rid, block := range globalRuntimeIDData {
		var s GeneralBlock
		decstr:=block[2].(string)
		decdata, _:=base64.StdEncoding.DecodeString(decstr)
		dec:=nbt.NewDecoder(bytes.NewBuffer(decdata))
	//}
	//for rid := 0; ; rid++ {
	//	var s GeneralBlock
		...
		s.Version=1
		// NO decoding version since it will fail ("Unexpected TAG_End")
	...
	fp, err = os.OpenFile("convert_out/blockmapping_nemc_2_1_10_mc_1_19_10_22.gob.brotli", os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0755)
