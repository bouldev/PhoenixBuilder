...
func DecodeSchem(data []byte, infoSender func(string)) (blockFeeder chan *IOBlock, cancelFn func(), suggestMinCacheChunks int, err error) {
	defer func() {
		r := recover()
		if r != nil {
	...
	for i := 0; i < dataLen; i++ {
		...
		if !rv.CanInt() {
			return nil, nil, 0, fmt.Errorf("cannot convert offset %v to int, please contact 2401PT", rv)
		}
	...
	if len(schemData.blockData) != int(schemData.Height)*int(schemData.Width)*int(schemData.Length) {
		return nil, nil, 0, fmt.Errorf("size check fail %v != %v", schemData.Width*schemData.Height*schemData.Length, len(schemData.blockData))
	}
	...
	airRID := chunk.AirRID
	fmt.Println(airRID)
	infoSender(fmt.Sprintf("格式匹配成功,开始解析,尺寸 [%v, %v, %v]\n", width, height, length))
	go func() {
		defer func() {
			close(blockChan)
		}()
