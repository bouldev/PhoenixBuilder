...
import (
	"fmt"
	"io"
	"phoenixbuilder/omega/components/omega_side/direct/connection"
	"phoenixbuilder/omega/components/omega_side/direct/packet"
		...
		for {
			...
			case clientPackets := <-clientPacketChan:
				...
				case *packet.EvalPBCommandPacket:
					handler.frame.FBEval(p.Command)
				case *packet.GameCommandPacket:
					if p.CommandType == packet.CommandTypeSettings {
						handler.frame.GetGameControl().SendWOCmd(p.Command)
						break
					} else if p.CommandType == packet.CommandTypeNormal {
						handler.frame.GetGameControl().SendCmdWithUUID(p.Command, p.UUID, true)
					} else {
						handler.frame.GetGameControl().SendCmdWithUUID(p.Command, p.UUID, false)
					}
				case *packet.GamePacket:
					handler.frame.GetGameControl().SendBytes(p.Content)
				...
				case *packet.UQHolderRequestPacket:
					...
					packet.SerializeAndSend(&packet.UQHolderResponsePacket{
						Content: handler.frame.GetUQHolder().Marshal(),
					}, conn)
				}
...
func ListenExt(frame defines.MainFrame, address string) (handlerStruct *ExternalConnectionHandler, err error) {
