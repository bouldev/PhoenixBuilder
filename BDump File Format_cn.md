 ...
 - 除此之外，原文中的一些，我认为排版不是很恰当的地方，我也稍作了一点点小小的改动，有兴趣的朋友可以对比一下[此文档的原文](BDump File Format.md)。
 - ——金羿
 - 翻译日期 2022 4 6， 原文日期 2022 1 29
...
| 2                 | `addX`                                     | **(强烈不推荐)** 将画笔的 `X` 坐标增加 `x`，顺带把画笔的 `Y` 和 `Z` 坐标重置为 `0`。 由于命令的实际功能与其名称之间存在差异，该方法在我们生成过程中不会再使用。虽然它已被我们弃用，但在读取 `bdx` 时仍然需要实现它的解析，因为包含此命令的 `bdx` 文件，也就是旧版本的文件一直都在。 | `unsigned short x`                                           |
| 3                 | `X++`                                      | **(强烈不推荐)** 将画笔的 `X` 坐标增加 `1`，顺带把画笔的 `Y` 和 `Z` 坐标重置为 `0`。（同上理） | -                                                            |
| 4                 | `addY`                                     | **(强烈不推荐)** 将画笔的 `Y` 坐标增加 `y`，顺带把画笔的 `Z` 坐标重置为 `0`。（同上理） | `unsigned short y`                                           |
| 5                 | `Y++`                                      | **(强烈不推荐)** 将画笔的 `Y` 坐标增加 `1`，顺带把画笔的 `Z` 坐标重置为 `0`。（同上理） | -                                                            |
| 6                 | `addZ`                                     | 将画笔的 `Z` 坐标增加 `z`，哦我的天哪，这竟然并没有被强烈不推荐，那是因为它并不会把什么东西搞没；可是，理所应当的，这玩意也不再在当下版本的 PhonixBuilder 中被支持了。 | `unsigned short z`                                           |
| 7                 | `placeBlock`                               | 在当前画笔的位置，带着方块数据 `blockData` 放置一个方块，此在方块池中的ID为 `blockID`。 | `unsigned short blockID`<br/>`unsigned short blockData`      |
| 8                 | `Z++`                                      | 将画笔的 `Z` 坐标增加 `1`，由于它并不会把什么东西搞没，所以也不强烈不推荐了；但这玩意也不再在当下版本的 PhonixBuilder 中被支持了。 | -                                                            |
| 9                 | `NOP`                                      | 摆烂。（不进行操作(No Operation)）                                   | -                                                            |
| 10, `0x0A`        | `jumpX`                                    | **(强烈不推荐)** Add `x` to the brush position's `X`, and reset the value of `Y` and `Z` to `0`. This method is deprecated since the difference between the real function of the command and what it's name say it should do. Though it's deprecated, you still need to implement it's read since there's still bdx files contain this command.<br/>The difference between `jumpX` and `addX` command is that `jumpX` uses `unsigned int` for its argument instead of `unsigned short`. | `unsigned int x`                                             |
| 11, `0x0B`        | `jumpY`                                    | **(强烈不推荐)** Add `y` to the brush position's `Y`, and reset the value of `Z` to `0`. | `unsigned int y`                                             |
