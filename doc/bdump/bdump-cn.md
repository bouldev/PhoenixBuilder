...
| 31, `0x1F`        | `useRuntimeIdPalette`                      | 使用预设的运行时ID方块池。<br/>`presetId`(预设ID) 是 PhoenixBuilder 内的值。网易MC( 1.17.0 @ 2.0.5 )下的 `presetId` 被我们定为 `117`。 每一个运行时ID都对应着一个方块，而且包含其数据值。<br/>相关内容详见 [PhoenixBuilder/resources](https://github.com/LNSSPsd/PhoenixBuilder/tree/main/resources)| `unsigned char presetId`                                     |
