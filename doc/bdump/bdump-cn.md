...
| 31, `0x1F`        | `useRuntimeIdPalette`                      | 使用预设的运行时ID方块池。`presetId`(预设ID) 是 PhoenixBuilder 内的值。网易我的世界基岩版( 1.17.0 @ 2.0.5 )下的 `presetId` 是 `117`。 每一个运行时ID都对应着一个方块，而且包含其数据值。<br/> 对于 `117` 版本，详见 [此](https://github.com/LNSSPsd/PhoenixBuilder/blob/main/resources/blockRuntimeIDs/netease/runtimeIds_117.json) 。<br/> 对于更多的可能正在使用的版本，详见 [此](https://github.com/LNSSPsd/PhoenixBuilder/tree/main/resources) 。 | `unsigned char presetId`                                     |
...
| 37, `0x25`        | `placeBlockWithChestData(uint16_t)`        | 在画笔所在位置放置一个 `runtimeId`(特定的运行时ID) 所表示的方块(如箱子、熔炉、唱片机等)，并在此方块处放入指定的物品。其中 `slotCount` 指的是 `要放入的物品数量` 。| `unsigned short runtimeId` <br/> `unsigned char slotCount` <br/> `struct ChestData data` |
| 38, `0x26`        | `placeBlockWithChestData`                  | 在画笔所在位置放置一个 `runtimeId`(特定的运行时ID) 所表示的方块(如箱子、熔炉、唱片机等)，并在此方块处放入指定的物品。其中 `slotCount` 指的是 `要放入的物品数量` 。| `unsigned int runtimeId`<br/>`unsigned char slotCount`<br/>`struct ChestData data` |
...
db 'E'
```
其中，`5240` 指的是 `正在燃烧的熔炉` 在 `ID` 为 `117` 的 `运行时ID方块池` 中的 `ID` ， 第一个 `2` 指的是要向 `正在燃烧的熔炉` 放置 `2` 个物品， `apple\x00` 指的是要放入 `苹果` ，`3` 指的是 `苹果` 的数量为 `3` ，首个 `(unsigned short)0` 指的是要放入的 `苹果` 的 `物品数据值` 为 `0` ，第二个 `(unsigned char)0` 指的是把 `苹果` 放在第 `1` 个槽位，`diamond\x00` 指的是要放入 `钻石` ，`64` 指的是 `钻石` 的数量为 `64` ，第三个 `(unsigned short)0` 指的是要放入的 `钻石` 的 `物品数据值` 为 `0` ，第二个 `2` 指的是把 `钻石` 放在第 `3` 个槽位。
