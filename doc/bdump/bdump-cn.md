...
> [Happy2018new](https://github.com/Happy2018new) 在使用 `BDX` 的过程中发现了本文档可能存在的一些问题，固然更新了本文档的一部分，主要是涉及 `箱子` 相关。<br>
...
| 31, `0x1F`        | `useRuntimeIdPalette`                      | 使用预设的运行时ID方块池。`presetId`(预设ID) 是 PhoenixBuilder 内的值。网易我的世界基岩版( 1.18.10 @ 2.1.10 )下的 `presetId` 是 `118`。 每一个运行时ID都对应着一个方块，而且包含其数据值。<br/> 相关内容可见 [PhoenixBuilder/tree/main/resources](https://github.com/LNSSPsd/PhoenixBuilder/tree/main/resources) 。 | `unsigned char presetId`                                     |
...
| 37, `0x25`        | `placeBlockWithChestData(uint16_t)`        | 在画笔所在位置放置一个 `runtimeId`(特定的运行时ID) 所表示的方块(如箱子、熔炉、唱片机等)，并在此方块处放入指定的物品。<br/>其中 `slotCount` 的数据类型为 `unsigned char`，因为我的世界用一个字节来存储物品栏编号，它指的是要放入的物品次数，即要载入的 `ChestData` 结构体数量。| `unsigned short runtimeId` <br/> `unsigned char slotCount` <br/> `struct ChestData data` |
| 38, `0x26`        | `placeBlockWithChestData`                  | 在画笔所在位置放置一个 `runtimeId`(特定的运行时ID) 所表示的方块(如箱子、熔炉、唱片机等)，并在此方块处放入指定的物品。<br/>其中 `slotCount` 的数据类型为 `unsigned char`，因为我的世界用一个字节来存储物品栏编号，它指的是要放入的物品次数，即要载入的 `ChestData` 结构体数量。| `unsigned int runtimeId`<br/>`unsigned char slotCount`<br/>`struct ChestData data` |
...
（下述内容的其中一部分目前未被更新，除去已经弃用的命令外，其余应当正常运作）
...
db 'E'
```
其中，`5240` 指的是 `正在燃烧的熔炉` 在 `ID` 为 `117` 的 `运行时ID方块池` 中的 `ID` ， 第一个 `2` 指的是要向 `正在燃烧的熔炉` 放入 `2` 次物品， `apple\x00` 指的是要放入 `苹果` ，`3` 指的是 `苹果` 的数量为 `3` ，首个 `(unsigned short)0` 指的是要放入的 `苹果` 的 `物品数据值` 为 `0` ，第二个 `(unsigned char)0` 指的是把 `苹果` 放在第 `1` 个槽位，`diamond\x00` 指的是要放入 `钻石` ，`64` 指的是 `钻石` 的数量为 `64` ，第三个 `(unsigned short)0` 指的是要放入的 `钻石` 的 `物品数据值` 为 `0` ，第二个 `2` 指的是把 `钻石` 放在第 `3` 个槽位。
