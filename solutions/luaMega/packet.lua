...
print("sleep 5s")
-- first sleep 5s
mux_poller:poll(omega.block.sleep, 5)
while mux_poller:has_next() do
    ...
    elseif event.type == omega.block.sleep then
        print("awake after 5s, poller begin to work!")
    ...
    elseif event.type == omega.block.user_input then
        ...
        mux_poller:poll(mux_poller.event_after, 3.5, "stop")                  -- make a single which occours after 3.5s
    elseif event.type == mux_poller.event_after and event.data == "stop" then
        print("stop packet poller")
