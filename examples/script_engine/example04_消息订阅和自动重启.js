...
game.listenChat(function (name, msg) {
    ...
    if (msg === "回城") {
        ...
        engine.sendCommand("tp " + name + " 0 100 0")
    }
    if (msg === "冒险") {
        engine.sendCommand("gamemode a " + name)
    }
    if (msg === "菜单") {
        engine.sendCommand("tellraw " + name + " {\"rawtext\":[{\"text\":\"输入 回城 以回到 0 100 0  \"}]}")
        engine.sendCommand("tellraw " + name + " {\"rawtext\":[{\"text\":\"输入 冒险 以切换为 冒险模式  \"}]}")
    }
})
