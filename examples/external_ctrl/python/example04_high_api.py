...
class Reactor(object):
    ...
    def on_text(self, msg: P.Text):
        if msg.Message == '§e%multiplayer.player.joined':
            ...
            return msg
        if msg.Message == "菜单":
            msg, _ = utils.pack_command(
                'tellraw ' + msg.SourceName + ' {"rawtext" : [{"text":"§6§l我们有苹果葡萄泡泡糖，请问您要什么呢？"}]}')
            return msg
        print(msg)
    ...
    def on_not_implemented_msg(self, packet_id, bytes_msg):
        if packet_id not in self.db['unknown_packet_type'].keys():
            self.db['unknown_packet_type'][packet_id] = bytes_msg
            print(f'new unknown packet type {packet_id} {bytes_msg}')
        return None
