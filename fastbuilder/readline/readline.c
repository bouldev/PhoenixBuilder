...
char **fb_readline_completion(const char *text, int start, int end) {
  ...
  if (strcmp("exit", command_pr) == 0 || strcmp("fbexit", command_pr) == 0 ||
      ...
      strcmp("ingameping", command_pr) == 0 || strcmp("set", command_pr) == 0 ||
      strcmp("setend", command_pr) == 0 || strcmp("logout", command_pr) == 0 ||
      strcmp("say", command_pr) == 0) {
    ...
    if (strcmp("--path", argv[argc - 1]) == 0 ||
        strcmp("-p", argv[argc - 1]) == 0 ||
        strcmp("-path", argv[argc - 1]) == 0) {
      free_converted_args(args);
      return rl_completion_matches(text, rl_filename_completion_function);
    } else if (strcmp("--facing", argv[argc - 1]) == 0 ||
               strcmp("-f", argv[argc - 1]) == 0 ||
               strcmp("-facing", argv[argc - 1]) == 0) {
      free_converted_args(args);
      return rl_completion_matches(text, fb_facing_enum_generator);
    } else if (strcmp("--shape", argv[argc - 1]) == 0 ||
               strcmp("-s", argv[argc - 1]) == 0 ||
               strcmp("-shape", argv[argc - 1]) == 0) {
      free_converted_args(args);
...
#define BUILD_GENERATOR(func_name, pool)                                       \
  static char *func_name(const char *text, int state) {                        \
...
const char *flags_pool[] = {"--assignnbtdata",
                            "-assignnbtdata",
                            "-nbt"
                            "--excludecommands",
                            "-excludecommands",
