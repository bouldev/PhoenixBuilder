		...
		for(unsigned int chunkZ=0;chunkZ<chunksLength;chunkZ++) {
			...
			for(unsigned int y=0;y<height;y++) {
				for(unsigned int cz=0;cz<16;cz++) {
					...
					if(chunkZ!=0) {
						gzseek(blockSources[cz], blocksBeginning+y*width*length+width*chunkZ*16+cz*width, SEEK_SET);
						gzseek(dataSources[cz], dataBeginning+y*width*length+width*chunkZ*16+cz*width, SEEK_SET);
					}
					for(unsigned cx=0;cx<16;cx++) {
						...
						builder_schematic_channel_input(channelID, realX+offset_x+beginX, y+offset_y+beginY, realZ+offset_z+beginZ, currentBlock, currentData);
						realX++;
					}
					realZ++;
					realX=baseX;
