...
func (s *SyncNewPlayerPhrasesData) MakeGo() (res any) {
	...
	for _, value := range s.Data {
		...
		if !ok {
			new[value.Source] = make(map[int64]any)
		}
		new[value.Source].(map[int64]any)[value.ID] = map[string]any{
...
func (s *SyncNewPlayerPhrasesData) FromGo(obj any) error {
	...
	// convert data
	{
		...
		for source_name, multiple_phrases := range phrases_data {
			mp, success := multiple_phrases.(map[int64]any)
			if !success {
				return fmt.Errorf(`FromGo: Failed to convert phrases_data[%#v] to map[int64]interface{}; phrases_data[%#v] = %#v`, source_name, source_name, multiple_phrases)
			}
			// convert data
