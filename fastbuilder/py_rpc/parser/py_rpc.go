...
func unmarshal(val []byte) (PyRpcObject, uint) {
	if val[0] == 0xc0 {
	...
	} else if (val[0] < 0x80) || (-0x20 <= int8(val[0]) && int8(val[0]) < 0) || (0xcc <= val[0] && val[0] <= 0xcf) || (0xd0 <= val[0] && val[0] <= 0xd3) {
		o := &PyRpcIntObject{}
		l := o.Parse(val)
		return o, l
		...
		l := o.Parse(val)
		return o, l
	} else if (0x90 <= val[0] && val[0] <= 0x9f) || val[0] == 0xdc || val[0] == 0xdd {
		o := &PyRpcListObject{}
		l := o.Parse(val)
		return o, l
	} else if (0x80 <= val[0] && val[0] <= 0x8f) || val[0] == 0xde || val[0] == 0xdf {
		o := &PyRpcMapObject{}
		l := o.Parse(val)
		return o, l
	} else {
