...
func (p *OnlineWorldProvider) LoadChunk(position world.ChunkPos) (c *chunk.Chunk, exists bool, err error) {
	...
	u_d, _ := uuid.NewUUID()
	err=command.SendCommand(fmt.Sprintf("tp %d 127 %d",100000-position[0],100000-position[1]),u_d,p.connection)
	if(err!=nil) {
	...
	u_d, _ = uuid.NewUUID()
	//command.PlayerTeleportSubscribers.Put(tr)
	err=command.SendCommand(fmt.Sprintf("tp %d 127 %d",position[0]*16,position[1]*16),u_d,p.connection)
	if(err!=nil) {
