...
func CreateClient(env *environment.PBEnvironment) *Client {
	secret_res, err:=http.Get(fmt.Sprintf("%s/api/new", args.AuthServer))
	if err!=nil {
...
func (client *Client) Auth(ctx context.Context, serverCode string, serverPassword string, key string, fbtoken string, username string, password string) (string, string, string, error) {
	...
	r, err:=client.client.Post(fmt.Sprintf("%s/api/phoenix/login", args.AuthServer), "application/json", bytes.NewBuffer(req_content))
	if err!=nil {
		panic(err)
	}
	...
	if len(respond_to)!=0&&len(args.CustomGameName)==0 {
		client.env.RespondUser=respond_to
	}
...
func (client *Client) TransferData(content string) string {
	r, err:=client.client.Get(fmt.Sprintf("%s/api/phoenix/transfer_start_type?content=%s", args.AuthServer, content))
	if err!=nil {
		panic(err)
	}
	resp:=assertAndParse(r)
...
func (client *Client) TransferCheckNum(data string) string {
	...
	r, err:=client.client.Post(fmt.Sprintf("%s/api/phoenix/transfer_check_num", args.AuthServer), "application/json", bytes.NewBuffer(msg))
	if err!=nil {
		panic(err)
	}
	resp:=assertAndParse(r)
